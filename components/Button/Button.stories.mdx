...
Contained buttons are identified by their color fill. The fill color in the
...
want to add an attribute, it is `contained`. Also, they cannot be combined with the
`outlined` attribute, because they belong to the _main button types_.

<Story name="Contained" default>
